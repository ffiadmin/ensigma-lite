...
//Grab the page data
	if (isset($_GET['id'])) {
		$id = $_GET['id'];
		$pageDataGrabber = mysql_query("SELECT * FROM `pages` WHERE `id` = '{$id}'", $connDBA);
		...
		if ($accepted == "true") {
			if ($pageData['published'] != "0") {
				if ($pageData['display'] == "1") {
					mysql_query("UPDATE `pages` SET `published` = '2', `display` = '2', `message` = '0' WHERE `id` = '{$id}'", $connDBA);
				} else {
					mysql_query("UPDATE `pages` SET `published` = '2', `display` = '1', `message` = '0' WHERE `id` = '{$id}'", $connDBA);
				}
			} else {
				mysql_query("UPDATE `pages` SET `published` = '2', `message` = '0' WHERE `id` = '{$id}'", $connDBA);
			}
			...
			if ($pageData['published'] != "0") {
				...
				mysql_query("UPDATE `pages` SET `message` = '1' WHERE `id` = '{$id}'", $connDBA);
			}
...
<p>&nbsp;</p>
<div class="toolBar">
  <a class="toolBarItem accept" href="approve_page.php?id=<?php echo $pageData['id']; ?>&accepted=true">Accept Pending Version</a>
  <?php
  //Only show if an update is pending
	  if ($pageData['published'] != "0") {
		  echo "<a class=\"toolBarItem reject\" href=\"approve_page.php?id=" . $pageData['id'] . "&accepted=false\">Revert to Currently Published</a>";
	  } else {
		  echo "<a class=\"toolBarItem reject\" href=\"approve_page.php?id=" . $pageData['id'] . "&accepted=false\">Reject Pending Version</a>";
	  }
  ?>
</div>
<br />
